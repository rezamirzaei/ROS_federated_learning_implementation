...
try:
    ...
    from fl_robots_interfaces.srv import GetModelInfo, RegisterRobot, TriggerAggregation
...
class AggregatorNode(BaseNode):  # type: ignore[misc,valid-type]
    """
    ...
    def __init__(self):
        ...
        if CUSTOM_INTERFACES:
            ...
            self.get_logger().info(
                "Services: register_robot, trigger_aggregation, get_global_model_info"
            )
    ...
    def _handle_trigger_aggregation(self, request, response):
        ...
        # hold the global state_lock around (it would block every other
        # callback, including incoming weight updates and heartbeat handling).
        with self.state_lock:
            min_p = request.min_participants if request.min_participants > 0 else self.min_robots
            pending = len(self.pending_weights)

        if pending < min_p and not request.force:
            response.success = False
            response.message = f"Not enough pending weights: {pending}/{min_p}"
    ...
    def _handle_get_model_info(self, request, response):
        ...
        response.current_round = self.current_round
        # Surface the latest aggregated round's mean-of-participants metrics.
        last_loss = 0.0
        last_accuracy = 0.0
        if self.aggregation_history:
            last_entry = self.aggregation_history[-1]
            losses = [l for l in last_entry.get("participant_losses", []) if l is not None]
            accuracies = [a for a in last_entry.get("participant_accuracies", []) if a is not None]
            if losses:
                last_loss = float(sum(losses) / len(losses))
            if accuracies:
                last_accuracy = float(sum(accuracies) / len(accuracies))
        response.last_loss = last_loss
        response.last_accuracy = last_accuracy
    ...
    def _handle_weight_update(self, robot_id: str, msg: String):
        ...
        try:
            ...
            # Drop stragglers: weights from a completed round must not pollute
            # the next aggregation window.
            if round_num < self.current_round:
                self.get_logger().warning(
                    f"Discarding stale weights from {robot_id} "
    ...
    def _perform_aggregation(self) -> dict | None:
        ...
        try:
            ...
            sample_counts = [
                self.pending_samples.get(rid, 1) for rid in self.pending_weights.keys()
            ]
