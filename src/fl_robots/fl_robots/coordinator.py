...
class CoordinatorNode(Node):
            ...
            with self.state_lock:
                if msg_type == 'registration':
                    ...
                    self.get_logger().info(
                        ...
                        f'Total: {len(self.registered_robots)}'
                    )

                elif msg_type == 'status':
                    if robot_id in self.registered_robots:
                        self.registered_robots[robot_id]['last_seen'] = time.time()
                        self.registered_robots[robot_id]['is_training'] = data.get('is_training', False)
    ...
    def _handle_waiting_for_robots(self):
        ...
        if active_robots >= self.config.min_robots:
            self.get_logger().info(
                f'{active_robots} robots ready, starting training'
            )
            self._start_training_round()
        else:
            self.get_logger().debug(
                f'Waiting for robots: {active_robots}/{self.config.min_robots}'
            )
    ...
    def _handle_training_round(self):
        ...
        if elapsed > self.config.round_timeout:
            self.get_logger().warning(
                f'Round {self.current_round} timed out after {elapsed:.1f}s'
            )
        ...
        if completed >= active_robots and completed >= self.config.min_robots:
            self.get_logger().info(
                f'Round {self.current_round}: All {completed} robots completed'
            )
