...
class SimulationEngine:
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            ...
            mean_tracking_error = sum(
                r.last_tracking_error for r in self.robots.values()
    ...
    def _reset_locked(self) -> None:
        ...
        for index in range(self.num_robots):
            ...
            self.robots[robot_id] = RobotState(
                ...
                formation_offset=offset, goal=offset,
    ...
    def _tick_locked(self) -> None:
        ...
        for robot_id, robot in robot_items:
            ...
            self.bus.publish(
                f"/fl/{robot_id}/mpc_plan", robot_id,
                {"goal": {"x": robot.goal[0], "y": robot.goal[1]},
                 "tracking_error": plan.tracking_error, "plan_cost": plan.cost},
            )
            self.bus.publish(
                f"/fl/{robot_id}/telemetry", robot_id,
        ...
        self.bus.publish(
            "/fl/coordinator_status", "coordinator",
            {"state": self.controller_state, "current_round": self.current_round,
             "training_active": self.training_active, "autopilot": self.autopilot},
        )
    ...
    def _update_training_locked(self, formation_error: float, min_separation: float) -> None:
        ...
        for index, robot in enumerate(self.robots.values(), start=1):
            ...
            self.bus.publish(
                f"/fl/{robot.robot_id}/model_weights", robot.robot_id,
                {"round": self.current_round + 1, "samples_trained": 256,
                 "loss": robot.training_loss, "accuracy": robot.accuracy},
            )
        ...
        if self.tick_count % 5 == 0:
            ...
            record = AggregationRecord(
                round_id=self.current_round, participants=len(self.robots),
                ...
                mean_divergence=divergence, formation_error=formation_error,
            )
