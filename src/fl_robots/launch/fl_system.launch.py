...
def generate_launch_description():
    ...
    total_rounds_arg = DeclareLaunchArgument(
        "total_rounds", default_value="20", description="Total number of training rounds"
    )

    learning_rate_arg = DeclareLaunchArgument(
        "learning_rate", default_value="0.001", description="Learning rate for local training"
    )

    batch_size_arg = DeclareLaunchArgument(
        "batch_size", default_value="32", description="Batch size for local training"
    )

    local_epochs_arg = DeclareLaunchArgument(
        "local_epochs", default_value="5", description="Number of local epochs per round"
    )

    enable_monitor_arg = DeclareLaunchArgument(
        "enable_monitor", default_value="true", description="Enable monitoring node"
    )

    enable_ui_arg = DeclareLaunchArgument(
        "enable_ui", default_value="true", description="Enable web dashboard and digital twin"
    )
